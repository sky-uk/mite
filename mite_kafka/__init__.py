...
@asynccontextmanager
async def _kafka_singleton_context_manager(
    ctx, singleton_id, producer, consumer, *args, **kwargs
):
    if producer:
        ctx.kafka_producer = await KafkaProducerSingleton.get_instance(
            ctx, singleton_id, *args, **kwargs
        )
    if consumer:
        # TODO: Implement
    ...
    *args,
    ...
    def wrapper_factory(func):
        ...
        async def wrapper(ctx, *args, **kwargs):
            ...
            if singleton:
                async with _kafka_singleton_context_manager(
                    ctx,
