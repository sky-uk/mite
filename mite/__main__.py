...
def _controller_log_end(logging_id, logging_url):
    ...
    if resp.status_code != 204:
        logger.warning(
            "Could not complete test end logging; " f"status was {resp.status_code}"
        )
...
def controller(opts):
    ...
    loop = asyncio.get_event_loop()
    logging_id = None
    if opts["--logging-webhook"] is not None:
    ...
    try:
        ...
        _controller_log_end(logging_id, opts["--logging-webhook"])
        loop.close()
